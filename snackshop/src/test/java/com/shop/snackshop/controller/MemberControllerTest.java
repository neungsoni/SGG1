...
@SpringBootTest
@AutoConfigureMockMvc//MockMvc테스를 위해 사용
@Transactional
...
class MemberControllerTest {
    ...
    //mockMvc 클래스를 이용해 실제 객체와 비슷하지만 테스트에 필요한 기능만 갖는 가짜객체.
    ...
    public Member createMember(String email, String password) {
        //로그인 진행을 위해 로그인 전 회원을 등록하는 메소드를 생성
    ...
    public void loginSuccessTest() throws Exception {
        ...
        mockMvc.perform(formLogin().userParameter("email")
                        ...
                        //회원가입 메소드를 실행 후 가입된 회원정보로 로그인이 되는지 테스트 진행
